...
このページでは、多項式の当てはめを題材にして、`LSQSolver` の基本的な使い方を説明します。多項式の当てはめとは、いくつかの点が与えられたときに、それらの点の近くを通る直線や曲線を求める問題です。たとえば、次のようなデータがあるとします。
...
問題の性質は、行列 $A$ の形とランクによって整理できます。行列 $A$ のランクを $r = \operatorname{rank}(A)$ と書きます。ここで、$r$ は、行列 $A$ が持っている独立な情報の数を表します。
