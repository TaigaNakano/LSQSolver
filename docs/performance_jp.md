...
次の表は、Octave におけます `backslash`、`pinv`、および QR 分解の計算時間を示しています。
...
1より大きいほど LSQSolver が高速ですことを表します。
...
以下の表では、LSQSolver の複数の実装版におけます elapsed time を比較しています。
